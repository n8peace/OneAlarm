...
if [ -z "$DEV_URL" ] || [ -z "$DEV_KEY" ] || [ -z "$PROD_URL" ] || [ -z "$PROD_KEY" ]; then
    ...
    echo ""
    echo "Example:"
    echo "  DEV_URL=https://your_dev_project.supabase.co DEV_KEY=your_dev_key PROD_URL=https://your_prod_project.supabase.co PROD_KEY=your_prod_key ./scripts/check-rls-simple.sh"
    exit 1
fi
