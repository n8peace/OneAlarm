-- Sync Triggers from Main to Develop (CORRECTED)
-- This migration makes develop triggers match main exactly
-- Uses the CORRECT develop URL
...
-- Create the exact main environment function but with DEVELOP URL
...
VALUES (
  'trigger_sync',
  jsonb_build_object(
    'action', 'sync_triggers_main_to_develop_corrected',
    ...
    'timestamp', NOW(),
    'note', 'Made develop triggers match main exactly - direct HTTP call to generate-audio with correct develop URL'
  )
); 