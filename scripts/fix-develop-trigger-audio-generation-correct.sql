...
-- This should call generate-audio for general audio, not queue alarm audio
  ...
  IF TG_OP = 'INSERT' OR 
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'environment', 'develop',
        'note', 'calling_generate_audio_directly_for_general_audio'
      )
    );

    -- Call generate-audio function for general audio (not alarm audio)
    -- This matches what main environment does
    PERFORM http_post(
...
VALUES (
  'trigger_fix',
  jsonb_build_object(
    'action', 'fix_develop_trigger_audio_generation_correct',
    'function_name', 'trigger_audio_generation',
    'purpose', 'Call generate-audio for general audio when preferences change',
    'environment', 'develop',
    'approach', 'direct_http_call_to_generate_audio',
    'note', 'This matches main environment behavior for user preferences',
    'timestamp', NOW()
  )
); 