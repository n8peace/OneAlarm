...
-- Replace the main version (with net.http_post) with develop version (queue-based)
...
VALUES (
  'trigger_fix',
  jsonb_build_object(
    'action', 'fix_develop_trigger_audio_generation',
    'function_name', 'trigger_audio_generation',
    'purpose', 'Replace main version (with net.http_post) with develop version (queue-based)',
    'environment', 'develop',
    'approach', 'queue_only_no_direct_http_no_net_extension',
    'note', 'This fixes the schema net does not exist error',
    'timestamp', NOW()
  )
); 