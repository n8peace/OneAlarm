...
# OneAlarm System Test Script
# Consolidated test script for different testing scenarios
...
show_usage() {
    echo -e "${BLUE}🔧 OneAlarm System Test Script${NC}"
    echo "====================================="
    echo ""
    echo "Usage: ./scripts/test-system.sh [TEST_TYPE] [SERVICE_ROLE_KEY]"
    echo ""
    echo "Test Types:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./scripts/test-system.sh quick YOUR_SERVICE_ROLE_KEY"
    echo "  ./scripts/test-system.sh e2e YOUR_SERVICE_ROLE_KEY"
    echo "  ./scripts/test-system.sh load YOUR_SERVICE_ROLE_KEY"
    echo ""
...
# If still no service role key, show usage
if [ -z "$SERVICE_ROLE_KEY" ]; then
    echo -e "${RED}❌ No service role key provided and SUPABASE_SERVICE_ROLE_KEY not set in environment${NC}"
    show_usage
fi
