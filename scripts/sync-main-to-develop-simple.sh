...
# This script completely copies main environment to develop
...
# Environment details
MAIN_URL="https://joyavvleaxqzksopnmjs.supabase.co"
MAIN_SERVICE_KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJmcnZhaHhtb2tleXJmbmxhaXdkIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MTQzMDI2NCwiZXhwIjoyMDY3MDA2MjY0fQ.C2x_AIkig4Fc7JSEyrkxve7E4uAwwvSRhPNDAeOfW-A"
...
echo -e "${BLUE}🔄 Complete Main → Develop Synchronization${NC}"
echo "================================================"
echo
echo -e "${YELLOW}⚠️  WARNING: This will completely overwrite the develop environment${NC}"
echo -e "${YELLOW}⚠️  All data, schema, and functions in develop will be replaced${NC}"
echo
...
confirm_action() {
    ...
    if [[ ! $REPLY =~ ^YES$ ]]; then
        print_status "error" "Synchronization cancelled"
        exit 1
    fi
}

# Step 1: Confirm the action
print_status "info" "Step 1: Confirming synchronization..."
confirm_action

# Step 2: Test connectivity
print_status "info" "Step 2: Testing connectivity to both environments..."

# Test main environment
MAIN_TEST=$(curl -s -w "\nHTTPSTATUS:%{http_code}" -X GET "${MAIN_URL}/rest/v1/" \
  -H "apikey: ${MAIN_SERVICE_KEY}" \
  -H "Authorization: Bearer ${MAIN_SERVICE_KEY}")

MAIN_HTTP_STATUS=$(echo "$MAIN_TEST" | grep "HTTPSTATUS:" | cut -d: -f2)

if [ "$MAIN_HTTP_STATUS" = "200" ]; then
    print_status "success" "Main environment accessible"
else
    print_status "error" "Cannot access main environment (HTTP $MAIN_HTTP_STATUS)"
    exit 1
fi

# Test develop environment
DEVELOP_TEST=$(curl -s -w "\nHTTPSTATUS:%{http_code}" -X GET "${DEVELOP_URL}/rest/v1/" \
  -H "apikey: ${DEVELOP_SERVICE_KEY}" \
  -H "Authorization: Bearer ${DEVELOP_SERVICE_KEY}")

DEVELOP_HTTP_STATUS=$(echo "$DEVELOP_TEST" | grep "HTTPSTATUS:" | cut -d: -f2)

if [ "$DEVELOP_HTTP_STATUS" = "200" ]; then
    print_status "success" "Develop environment accessible"
else
    print_status "error" "Cannot access develop environment (HTTP $DEVELOP_HTTP_STATUS)"
    exit 1
fi

# Step 3: Create comprehensive synchronization SQL
print_status "info" "Step 3: Creating comprehensive synchronization SQL..."
...
# Step 5: Create storage synchronization instructions
print_status "info" "Step 4: Creating storage synchronization instructions..."
...
print_status "info" "Step 5: Creating deployment instructions..."
...
# Step 7: Summary
print_status "info" "Step 6: Synchronization Summary"
echo "====================================="
echo "• Main environment: ✅ Accessible"
echo "• Develop environment: ✅ Accessible"
echo "• Complete schema SQL: ✅ $SYNC_FILE"
echo "• Storage instructions: ✅ $STORAGE_SYNC_FILE"
echo "• Deployment guide: ✅ $DEPLOY_FILE"
echo

print_status "success" "🎉 Complete synchronization preparation finished!"
echo
...
echo "4. Test the synchronization"
echo
