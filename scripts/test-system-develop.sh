...
# OneAlarm System Test Script (Development)
# This script matches main, except for environment variables
...
# Develop environment variables (override config.sh)
SUPABASE_URL="https://xqkmpkfqoisqzznnvlox.supabase.co"
SUPABASE_PROJECT_REF="xqkmpkfqoisqzznnvlox"
...
show_usage() {
    echo -e "${BLUE}🔧 OneAlarm System Test Script (Development)${NC}"
    echo "====================================="
    echo ""
    echo "Usage: ./scripts/test-system-develop.sh [TEST_TYPE] [SERVICE_ROLE_KEY]"
    echo ""
    echo "Test Types:"
    echo "  quick     - Quick system health check (1 user, basic functionality)"
    echo "  e2e       - End-to-end test (3 users, full workflow)"
    echo "  load      - Load test (50 users, performance validation)"
    echo "  tz        - Multi-timezone test (timezone handling)"
    echo "  audio     - Audio generation test"
    echo "  queue     - Queue processing test"
    echo ""
    echo "Examples:"
    echo "  ./scripts/test-system-develop.sh quick YOUR_SERVICE_ROLE_KEY"
    echo "  ./scripts/test-system-develop.sh e2e YOUR_SERVICE_ROLE_KEY"
    echo "  ./scripts/test-system-develop.sh load YOUR_SERVICE_ROLE_KEY"
    echo ""
...
EOF
    ...
    USER_HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$SUPABASE_URL/rest/v1/users" \
        -H "Authorization: Bearer $SERVICE_ROLE_KEY" \
        -H "apikey: $SERVICE_ROLE_KEY" \
        -H "Content-Type: application/json" \
        -H "Prefer: return=representation" \
...
run_e2e_test() {
    set -x
    echo -e "${BLUE}🚀 End-to-End Test (3 Users)${NC}"
    echo "====================================="
    echo ""
