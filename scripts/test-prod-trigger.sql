-- Test production trigger and function status
-- This script verifies the user_preferences_audio_trigger is working in production
...
-- 6. Check for trigger restoration log
SELECT 
    'Trigger restoration log' as check_item,
    created_at,
    meta
FROM logs 
WHERE event_type = 'trigger_restoration' 
    AND meta->>'action' = 'restore_prod_user_preferences_audio_trigger'
ORDER BY created_at DESC
LIMIT 1; 