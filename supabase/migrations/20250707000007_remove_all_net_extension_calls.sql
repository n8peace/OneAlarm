-- Remove ALL net extension calls from develop environment
-- This migration ensures no functions use the net extension
  ...
  IF TG_OP = 'INSERT' OR 
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'environment', 'develop',
        'note', 'queue_only_no_direct_http_no_net_extension'
      )
    );
...
-- Check for any other functions that might use net extension and drop them
-- This is a safety measure to ensure no net extension calls remain
DO $$
DECLARE
    func_record RECORD;
BEGIN
...
VALUES (
  'trigger_update',
  jsonb_build_object(
    'action', 'remove_all_net_extension_calls',
    'trigger_name', 'on_preferences_updated_and_inserted',
    'function_name', 'trigger_audio_generation',
    'purpose', 'Remove ALL net extension calls from develop environment',
    'environment', 'develop',
    'approach', 'queue_only_no_direct_http_no_net_extension',
    'note', 'Completely eliminated net extension dependency',
    'timestamp', NOW()
  )
); 